...
기준: **2026-09-11, v0.1.49**. Claude 등 다른 서비스가 원래 대화를 읽지 않아도 이어서 작업하기 위한 문서다. 저장소는 `yunhyok/Remote-Control-App`이며 사용자 승인에 따라 **Public으로 전환했다.** 개발을 처음부터 재시작하지 않는다.
...
- **v0.1.49에서 네 가지를 추가했고 모두 구현됨·현장 미검증이다.** ① 학습한 위치 자동 복사(수동 복사 1회로 클릭 지점을 배우고 같은 실행의 화면에서 확인한 뒤, 다음 실행에서 클릭 1회+Ctrl+A/C 1회), ② 본문 탐색 진단 `B2|…`와 `OUTPUT_REGION_UNCONFIRMED` 코드, ③ 전사본과 전체 텍스트의 줄 단위 **원문 대조**, ④ 사용자가 버튼으로만 만드는 **진단 묶음 ZIP**.
...
Slave의 캡처/판독 자체는 여전히 창 활성화나 키보드·마우스 입력을 하지 않는다. **v0.1.49의 자동 복사 경로만이 Slave의 유일한 입력 경로다.** 이 경로는 학습한 위치를 현장에서 다시 확인한 뒤 클릭 1회와 Ctrl+A/Ctrl+C 1회를 보내고, 커서를 원래 위치로 되돌리며 클립보드는 복원하지 않는다. 설정에서 끌 수 있고, 끄면 이전 버전과 동일하게 무입력이다. 저장된 좌표를 그대로 클릭하거나 다른 pane·과거 클립보드를 성공으로 채택하지 않는다.
...
| v0.1.49 | 학습한 위치 자동 복사, 본문 탐색 진단 B2, 원문 대조, 진단 묶음 | 수동 복사 1회가 위치를 가르친다. 네 기능 모두 현장 미검증 |
...
### 자동 복사와 위치 학습 (v0.1.49, 현장 미검증)

`OutputAutoCopy`는 Slave의 **유일한 입력 경로**다. 학습은 수동 복사 성공 직후 `--powersi-output-copy` worker가 전경 창이 대상 root(또는 그 자식)인지, 물리 커서가 client 안인지를 확인해 `A1|pid|시작ticks|세션|x|y|client폭|높이|학습시각`을 OB1 detail 뒤에 덧붙이는 것으로 이뤄진다. UI는 **같은 실행의 캡처 프레임에서 그 점을 포함하는 본문을 `OutputPaneImage.FindBodyAt`로 다시 찾고 client 크기가 프레임 크기와 같을 때만** 저장한다(`OUTPUT_ANCHOR_LEARNED`). 아니면 `OUTPUT_ANCHOR_UNCONFIRMED reason=FRAME_MISSING|SIZE_MISMATCH|BODY`로 남고 자동 복사에 쓰지 않는다. 위치는 메모리 전용이며 디스크에 쓰지 않는다.

실행은 별도 worker(최대8초)에서 한다. 순서는 `ResolveWindow`로 HWND 재확인 → pid·시작시각·세션·client 크기 일치, 표시/최소화 확인(`AUTO_COPY_WINDOW_CHANGED`) → 새 캡처에서 `FindBodyAt`(`AUTO_COPY_BODY_UNCONFIRMED`, detail은 `B2|…`) → 마우스/Ctrl/Shift/Alt/Win 눌림 없음(`AUTO_COPY_INPUT_BUSY`), 클릭 지점의 root가 대상(`AUTO_COPY_OCCLUDED`), `SM_SWAPBUTTON` 확인 → 커서 이동·재확인(`AUTO_COPY_CURSOR_NOT_SET`) → 클릭 1회 → 전경 대기 600ms(`AUTO_COPY_FOREGROUND_FAILED`), 커서 유지 확인(`AUTO_COPY_CURSOR_MOVED`) → 스캔코드 `SendInput`으로 Ctrl+A, 80ms, Ctrl+C(각 chord 전에 전경 재확인, `AUTO_COPY_FOREGROUND_LOST`, 모든 키는 finally에서 해제) → 새 클립보드 sequence 2초 대기(`AUTO_COPY_NO_CLIPBOARD`) → 기존 소유자 검사 재사용(`AUTO_COPY_CLIPBOARD_FOREIGN`). 성공은 `AUTO_COPY_READ / AUTO_CLIPBOARD`, detail `A2|학습x|y|본문x|y|w|h|클릭화면x|y|B2|…`다. 커서는 끝에서 되돌리고 **클립보드는 복원하지 않는다.** 실패하면 UI가 클립보드 기준값을 다시 읽고 기존 30초 수동 안내로 넘어간다.
...
비교 화면의 전사본 아래에는 v0.1.49의 **원문 대조**가 있다. `TranscriptComparison.Compare(전체 텍스트, 전사본)`가 전사본 각 줄을 전체 텍스트 끝 600줄과 맞춰 EXACT/NORMALIZED/NEAR/MISSING/EMPTY와 순서 유지를 계산하고, 화면에는 `Render()`(본문 포함), 로그에는 `T1|…`만 남는다. 같은 창의 **진단 묶음 저장…**은 경고 확인과 저장 위치 지정을 거쳐 ZIP(README/manifest/full-text/slave-log/실행별 이미지·전사본·대조)을 만든다. 자동 생성·자동 전송은 없다.

`VISION_RUN`은 모델 ID/이름/키/양자화, 전체 `sample`과 실제 `ocr_sample` SHA256, `mode`, `crop_reused`, `request_timeout_s`, `total_ms/locate_ms/read_ms`, `geometry=P2|제안x|y|w|h|본문x|y|w|h|OCR폭|높이`, v0.1.49의 `frame=폭x높이`와 `body=B2|화면폭|높이|후보|채택|크기|채움|과대|중심|겹침`을 기록한다. 미확인 필드는 UNKNOWN. OCR-only면 locate_ms=0이다. read_ms에는 모델 목록/HTTP 대기도 포함되어 순수 GPU 속도가 아니다. 그 밖의 v0.1.49 줄은 `OUTPUT_AUTO_COPY_BEGIN`, `OUTPUT_AUTO_COPY_FAILED`, `OUTPUT_ANCHOR_LEARNED/UNCONFIRMED`, `TRANSCRIPT_COMPARE`, `DIAG_BUNDLE_SAVED/FAILED`이며 모두 고정 코드·정수·해시만 담는다(`SlaveLog.Write(code, detail)`가 `[A-Za-z0-9_=| ]`로 제한).
...
가장 먼저 할 일은 **v0.1.49의 자동 복사와 진단 묶음을 한 번의 Slave 현장 시험으로 검증하는 것**이다. [SLAVE-TEST.md](SLAVE-TEST.md)의 여섯 단계가 그 절차다. 동작한다고 미리 보고하지 않는다. 확인할 것은 ① 두 번째 **두 방식 비교**에서 `AUTO_COPY_READ`가 나오는지, 아니면 어떤 `AUTO_COPY_*`로 중단되는지, ② 자동 복사본이 수동 복사본과 같은 규모인지(Output 전체 선택 여부), ③ PowerSI 쪽에 부작용이 없는지다. 실패 코드가 나오면 그 코드가 가리키는 단계만 고친다. 임의의 키를 운영 중인 PowerSI에 직접 시험해 개발 검증을 대신하지 않는다.

그 다음은 **진단 묶음이 알려주는 것에 따른 보정**이다. v0.1.48 현장 결과처럼 경계 확정이 계속 실패하면, `B2|…` 후보/탈락 수치와 묶음 안의 전체 화면·제안 영역 이미지를 함께 보고 (a) 모델 제안 품질, (b) 평평한 중립색 배경 휴리스틱의 한계, (c) 학습된 클릭 위치를 경계 탐색의 기준으로 쓰는 방식 중 무엇을 고칠지 정한다. 자동 복사가 되면 본문 경계 없이도 전체 원문은 확보되므로, OCR 경로는 “원문이 없는 다른 앱”을 위한 독립 경로로 계속 유지한다. 원문 대조 결과로 전사 정확도를 처음으로 수치화할 수 있다.
...
| `OutputAutoCopy.cs` | v0.1.49 자동 복사 worker·위치 학습/직렬화·입력 주입. 이 앱의 유일한 입력 코드 |
| `DiagnosticBundle.cs` | v0.1.49 진단 묶음 ZIP 작성기. 사용자 버튼으로만 호출되며 자동 실행 없음 |
| `TranscriptComparison.cs` | v0.1.49 전사본 대 전체 텍스트 줄 단위 대조(순수 계산, UI/IO 없음) |
...
스크립트는 각 대상 restore/build Release → 실제 EXE `--self-test` → 종료 코드 확인 → ZIP/SHA256 생성을 한다. 자체 검사에는 모의 loopback 서버, 프로토콜/인증/모델 응답/픽셀/재판독/취소/UI 상태 검사 등이 있다. 실사용 앱 창에 입력하는 시험과 구분한다. `dist/verification-v0.1.49/`의 stdout/stderr로 결과를 확인한다. 필요한 변경이 있을 때만 기존 추가 개발용 `scripts/test-powersi-capture.ps1`, `test-powersi-discovery.ps1` 등을 실행한다. `test-layout-replay.ps1`은 별도 현장 로그가 필요한 과거 KI 재현용이며 일반 빌드 필수조건이 아니다.
...
패키지: 전체는 `dist/Remote-Monitor-v0.1.49-win7-win11-net48.zip`, Slave-only는 `dist/Remote-Monitor-Slave-v0.1.49-win11-net48.zip`. EXE/config를 함께 둔다. 프로그램명/버전은 제목 표시줄에서 확인한다. Master를 연결할 때는 양쪽 버전을 맞추며 **현재 사용자 시험에서는 기존 Master를 바꾸거나 연결할 필요가 없다.**

현재 현장 절차는 SLAVE-TEST의 여섯 단계가 기준이다. 요약하면 v0.1.49 확인 → E4B로 두 방식 비교1회와 안내 시 수동 복사1회(위치 학습) → 두 방식 비교1회를 조작 없이 실행해 `AUTO_COPY_READ` 확인 → OCR 입력이 있으면31B Q4·90초로 같은 화면 재판독1회 → 비교 창에서 원문 대조 확인 → 진단 묶음 저장 후 로그와 ZIP 전달이다. Q8 반복/새 resize/모바일 재시험을 추가하지 않는다. 105초 초과 시 Stop과 로그 전달이며 무한 대기는 하지 않는다. 모델 다운로드·동시 로드·GPU나 시뮬레이션 설정 자동 변경은 요구하지 않는다.
...
진단 묶음 정책: ZIP은 **사용자가 비교 화면의 버튼을 눌렀을 때만** 만들어지고, 경고 확인과 저장 위치 지정을 거친다. 자동 생성·자동 전송·주기적 수집은 없다. 안에는 화면 이미지와 Output 전체 텍스트·전사본·대조 결과가 그대로 들어가므로 **Git 저장과 외부 공유 대상이 아니며** 전달 여부는 소유자가 정한다. README.txt가 이 경고를 담고, 로그에는 크기와 SHA256만 남기고 경로는 남기지 않는다.
...
기준 버전은 v0.1.49입니다. 모바일/Win7 Master/Win11 Slave 통신은 현장 검증되어
...
v0.1.49의 학습한 위치 자동 복사·원문 대조·진단 묶음은 구현됐지만 현장 결과는 아직 없습니다.
다음 작업은 이 세 기능을 한 번의 Slave 현장 시험으로 확인하고,
